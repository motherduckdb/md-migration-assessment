...
the M3b workload facts (``report.spend_profile``, ``report.workload_profile``,
``report.ingestion_inventory``), and the M3c aggregate facts
(``report.concurrency_profile``, ``report.tool_fingerprints``,
``report.query_shapes``, ``report.workload_rollup``,
``report.dialect_constructs``) for every collection in the database. Facts
only — no compatibility ratings, no effort scores; those are applied by the
internal overlay.
...
def build_report(con: duckdb.DuckDBPyConnection) -> dict:
    ...
    _build_concurrency_profile(con)
    _build_tool_fingerprints(con)
    _build_query_shapes(con)
    _build_workload_rollup(con)
    _build_dialect_constructs(con)
    return summary
...
def _build_concurrency_profile(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if not _table_exists(con, "raw", "query_concurrency"):
        return
    con.execute("""
...
def _build_tool_fingerprints(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if _table_exists(con, "raw", "client_app_fingerprints"):
        con.execute("""
        ...
        """)
    if _table_exists(con, "raw", "query_tag_fingerprints"):
        con.execute("""
            INSERT INTO report.tool_fingerprints BY NAME
            SELECT
                f.collection_id AS collection_id,
        ...
        """)
    if _table_exists(con, "raw", "login_history"):
        con.execute("""
            INSERT INTO report.tool_fingerprints BY NAME
            SELECT
                l.collection_id AS collection_id,
...
def _build_query_shapes(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if not _table_exists(con, "raw", "query_shapes"):
        return
    con.execute("""
...
def _build_workload_rollup(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if not _table_exists(con, "raw", "query_workload_rollup"):
        return
    con.execute("""
...
def _build_dialect_constructs(con: duckdb.DuckDBPyConnection) -> None:
    ...
    if not _table_exists(con, "raw", "query_dialect_constructs"):
        return
